...
export function CourseCard({
  ...
  return (
    <article className="group relative flex flex-col overflow-hidden rounded-2xl border border-brand-border bg-brand-surface transition-all duration-300 will-change-transform hover:-translate-y-1 hover:shadow-xl hover:shadow-brand-primary/5">
      {/* Thumbnail */}
      <Link
        href={`/courses/${slug}`}
      ...
      >
        ...
        {/* Price badge overlay */}
        {price === 0 && (
          <Badge className="absolute left-3 top-3 rounded-lg bg-brand-accent text-xs font-semibold text-white">
            უფასო
          </Badge>
        )}
      </Link>
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        {/* Meta */}
        <div className="mt-4 flex items-center gap-3 text-xs text-brand-muted">
